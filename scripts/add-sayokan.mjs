...
async function addToSqlite(dbPath) {
  ...
  if (existing) {
    console.log(`[SQLite] Sayokan branşı (${dbPath}) zaten mevcut:`, existing);
    return true;
  }
...
async function addToPocketBaseApi() {
  ...
  try {
    ...
    if (existing) {
      console.log('[PB API] Sayokan branşı zaten PocketBase API üzerinde mevcut.');
      return;
    }
...
async function main() {
  console.log('--- Sayokan Branşı Ekleme Başlatılıyor ---');
  const targetDb =
    process.argv[2] ||
