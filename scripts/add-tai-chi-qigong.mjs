...
async function addToSqlite(dbPath) {
  ...
  if (existing) {
    console.log(`[SQLite] Tai Chi & Qigong branşı (${dbPath}) zaten mevcut:`, existing);
    return true;
  }
...
async function addToPocketBaseApi() {
  ...
  try {
    ...
    if (existing) {
      console.log('[PB API] Tai Chi & Qigong branşı zaten PocketBase API üzerinde mevcut.');
      return;
    }
...
async function main() {
  console.log('--- Tai Chi & Qigong Branşı Ekleme Başlatılıyor ---');
  const targetDb =
    process.argv[2] ||
