...
- **Mid-stream script degisikligi**: `bash` calisan script dosyasini disktan satir satir okur. `git pull` `deploy/server-auto-update.sh`'i degistirirse satir kaymasi olusur; `exec` satirina gelinmeden eski `LoadState`/grep kodu calisabiliyordu. Cozum: betik en basta kendini `/tmp/spornerede-autoupdate.*.sh` altina kopyalayıp `exec bash` ile oradan calisir (`SN_AUTOUPDATE_TMP_RUN`); pull repo dosyasini guncellerken /tmp kopyasi degismez.
