...
- **`spornerede.service bulunamadi, restart atlandi` ama unit aslinda var**: Eski script `systemctl list-unit-files | grep -q` kullanirken `pipefail` altinda SIGPIPE / `Broken pipe` olusabiliyordu; `LoadState` on kontrolu de bazi kosullarda bos cikabiliyordu. Guncel `deploy/server-auto-update.sh` dogrudan `systemctl restart spornerede.service` dener (basarisizsa uyari loglar). Sorun devam ederse: `systemctl restart spornerede` ile dogrula.
- **Mid-stream script degisikligi**: `bash` calisan script dosyasini disktan satir satir okur. `git pull` islemi `deploy/server-auto-update.sh`'in kendisini degistirirse, sonraki satirlar yeni dosyadan okunur ve **ofset kaymasi nedeniyle bash beklenmedik bir yerden devam edebilir** (gercek olay: pull eski `list-unit-files | grep` koduna kayan satira gelmis ve fix devre disi kalmisti). Bu yuzden `git pull` sonrasi script kendini `exec bash $0` ile yeniden baslatir; `AUTO_UPDATE_RESPAWNED=1` flag'i sonsuz dongu ve cift pull'u engeller. Bu kalip degistirilmemeli.
